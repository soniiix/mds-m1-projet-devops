...
describe('createTask', () => {
  it('should create a task with injected id and date', () => {
    const dto = { title: 'My task' };
    const fixedId = 'test-id-123';
    const fixedDate = new Date('2024-01-01');

    const task = createTask(dto, fixedId, fixedDate);

    expect(task.id).toBe(fixedId);
  ...
  it('should create a task with default id and date when not provided', () => {
    const dto = { title: 'My task' };
  ...
  it('should trim title and description', () => {
    ...
    const task = createTask(dto, 'id-1', new Date());
  ...
  it('should throw error for invalid title', () => {
    expect(() => createTask({ title: '' }, 'id-1', new Date())).toThrow(
      'Title cannot be empty',
    );
    expect(() =>
      createTask({ title: 'a'.repeat(101) }, 'id-1', new Date()),
  ...
  it('should be deterministic with same inputs', () => {
    ...
    const task1 = createTask(dto, id, date);
    const task2 = createTask(dto, id, date);
...
describe('updateTask', () => {
  ...
  it('should throw error for invalid title', () => {
    expect(() => updateTask(baseTask, { title: '' })).toThrow(
      'Title cannot be empty',
    );
  });
